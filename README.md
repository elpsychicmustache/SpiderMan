...
This is a safe, CLI manual web application directory spidering program built using Python. The reason it is safe is because it does not use automatic scripts to discover directories, nor does it automatically populate forms. This prevents the risk of unintentionally causing changes to the web application.
...
`python3 src/spiderman.py -o "output.txt"`
...
4. From the project's root folder, run `python src/spiderman.py` or `python3 src/spiderman.py` to get started. 
...
- spiderman.py: This is the main entry point for the program.
